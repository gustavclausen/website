...
function convertTextToHtmlParagraph(text) {
    const textHtml = text
        ...
        .replace(/\[([^\s]+)\]\(((https|http|mailto):[^\s]+)\)/g, "<a href='$2' target='_blank'>$1</a>")
        // Add linebreaks
        .split('\n')
        .join("<br/>");
...
async function executeCommand(linesEl, commandObj) {
    const emptyCommandLine = $(linesEl).children().last().find('.commandLine');
    const command = commandObj.command;
    const commandOutput = commandObj.output.fun ? await commandObj.output.fun() : null;
    const clearLines = commandObj.clear;

    const typeCommandCharacters = function (chars) {
        return chars.reduce((prom, _, i) => {
            return prom.then(() => emptyCommandLine.html(command.substr(0, i + 1))).delay(charTypeSpeed);
        }, Promise.resolve()); // initial
    };
    await typeCommandCharacters(command.split(''));

    if (commandOutput) {
        await new Promise(res => res()).then(() => removeCursor()).delay(commandObj.output.delay).then(() => $(linesEl).append(convertTextToHtmlParagraph(commandOutput)));
    }

    if (clearLines) {
        await new Promise(res => res()).delay(commandObj.output.delay).then(() => $(linesEl).empty());
    }
